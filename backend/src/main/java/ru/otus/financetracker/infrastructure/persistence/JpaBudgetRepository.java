...
public class JpaBudgetRepository implements BudgetRepository {
    ...
    public JpaBudgetRepository(BudgetJpaRepository budgetJpaRepository) {
    ...
    public Budget save(Budget budget) {
        ...
        return budgetJpaRepository.findByIdAndUserId(budget.id(), budget.userId())
                .map(this::toDomain)
                .orElseThrow();
    }

    @Override
    public Optional<Budget> findByIdAndUserId(UUID budgetId, UUID userId) {
        return budgetJpaRepository.findByIdAndUserId(budgetId, userId).map(this::toDomain);
    }

    @Override
    public Page<Budget> findAllByUserId(UUID userId, Pageable pageable) {
        return budgetJpaRepository.findAllByUserId(userId, pageable).map(this::toDomain);
    }

    @Override
    public void delete(Budget budget) {
        budgetJpaRepository.delete(toEntity(budget));
        budgetJpaRepository.flush();
    }
