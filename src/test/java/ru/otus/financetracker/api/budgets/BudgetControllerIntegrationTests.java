...
class BudgetControllerIntegrationTests {
    ...
    @AfterEach void clearData() { transactionTemplate.executeWithoutResult(status -> {
    ...
    @Test void shouldCreateListGetUpdateAndDeleteBudget() throws Exception {
        ...
        mockMvc.perform(patch("/api/v1/budgets/{id}", budgetId).header("Authorization", "Bearer " + token)
                        .contentType("application/json").content("{\"version\":0,\"limitAmount\":600.0000,\"currency\":\"USD\"}"))
                .andExpect(status().isOk()).andExpect(jsonPath("$.limitAmount").value("600.0000")).andExpect(jsonPath("$.version").value(1));
        mockMvc.perform(delete("/api/v1/budgets/{id}?version=1", budgetId).header("Authorization", "Bearer " + token))
                .andExpect(status().isNoContent());
    }
    @Test void shouldRejectInvalidMonthLimitIncomeAndDuplicateBudget() throws Exception {
        String token = registerAndLogin("budget-validation@example.test"); String expense = createCategory(token, "EXPENSE"); String income = createCategory(token, "INCOME");
        mockMvc.perform(post("/api/v1/budgets").header("Authorization", "Bearer " + token).contentType("application/json")
                        .content(request(expense, "2026-09-02", "500.0000"))).andExpect(status().isBadRequest()).andExpect(jsonPath("$.code").value("VALIDATION_FAILED"));
        mockMvc.perform(post("/api/v1/budgets").header("Authorization", "Bearer " + token).contentType("application/json")
    ...
    @Test void shouldRequireAuthenticationForBudgets() throws Exception {
        mockMvc.perform(get("/api/v1/budgets")).andExpect(status().isUnauthorized());
    }
    @Test void shouldHideForeignBudgetAndRejectStaleUpdateAndDelete() throws Exception {
        String owner = registerAndLogin("budget-owner2@example.test"); String other = registerAndLogin("budget-other@example.test");
    ...
    private String registerAndLogin(String email) throws Exception {
        mockMvc.perform(post("/api/v1/auth/register").contentType("application/json").content("{\"email\":\"%s\",\"password\":\"a-secure-password\",\"displayName\":\"Test User\",\"baseCurrency\":\"USD\"}".formatted(email))).andExpect(status().isCreated());
        var response = mockMvc.perform(post("/api/v1/auth/login").contentType("application/json").content("{\"email\":\"%s\",\"password\":\"a-secure-password\"}".formatted(email))).andExpect(status().isOk()).andReturn();
        return new tools.jackson.databind.ObjectMapper().readTree(response.getResponse().getContentAsString()).get("accessToken").asText();
    }
}
