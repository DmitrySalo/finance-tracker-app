...
class BudgetExpenseAggregationIntegrationTests {
    ...
    @Test void shouldAggregateOnlyCurrentUsersCategoryExpensesWithinBudgetMonthUsingPersistedRates() {
        UUID userId = insertUser();
        UUID categoryId = insertCategory(userId, "EXPENSE");
        UUID anotherCategoryId = insertCategory(userId, "EXPENSE");
        UUID anotherUserId = insertUser();
        UUID anotherUserCategoryId = insertCategory(anotherUserId, "EXPENSE");
        ...
        BigDecimal spentAmount = transactionRepository.sumExpenseAmountInBaseCurrency(
    ...
    @Test void shouldCalculateListedBudgetsUsingBoundedMonthlyAggregations() {
        UUID userId = insertUser();
        UUID septemberCategoryId = insertCategory(userId, "EXPENSE");
        UUID octoberCategoryId = insertCategory(userId, "EXPENSE");
        budgetService.create(userId, new CreateBudgetCommand(septemberCategoryId, LocalDate.of(2026, 9, 1),
                new BigDecimal("100.0000"), "USD"));
        budgetService.create(userId, new CreateBudgetCommand(octoberCategoryId, LocalDate.of(2026, 10, 1),
                new BigDecimal("100.0000"), "USD"));
        ...
        assertThat(calculations).extracting(calculation -> calculation.budget().budgetMonth(), calculation -> calculation.spentAmount())
    ...
    private UUID insertCategory(UUID userId, String transactionType) {
        ...
        jdbcTemplate.update("INSERT INTO categories (id,user_id,name,transaction_type,icon,color) VALUES (?,?,?,?,?,?)",
                id, userId, id.toString(), transactionType, "icon", "#0A1B2C");
        return id;
    }

    private void insertTransaction(UUID userId, UUID categoryId, String amount, String currency, String exchangeRate,
        ...
        jdbcTemplate.update("""
                        ...
                        VALUES (?,?,?,?,?,?,?,?)
                        """, UUID.randomUUID(), userId, categoryId, new BigDecimal(amount), currency,
                new BigDecimal(exchangeRate), transactionDate, "EXPENSE");
    }
}
