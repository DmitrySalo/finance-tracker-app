...
public class RecurringTransactionService {
    ...
    public RecurringTransaction create(UUID userId, CreateRecurringTransactionCommand command) {
        ...
        return recurringTransactionRepository.save(new RecurringTransaction(UUID.randomUUID(), userId, command.categoryId(),
                command.amount(), command.currency(), command.exchangeRateToBase(), strip(command.description()),
                command.transactionType(), command.dayOfMonth(), command.startDate(),
                firstOccurrence(command.startDate(), command.dayOfMonth()), command.active(), now, now, 0));
    }

    @Transactional(readOnly = true)
    public RecurringTransaction get(UUID userId, UUID id) { return findOwned(userId, id); }
    ...
    public RecurringTransaction update(UUID userId, UUID id, UpdateRecurringTransactionCommand command) {
        ...
        if (current.version() != command.version()) throw new OptimisticLockingFailureException("Recurring transaction has been modified.");
        ...
        LocalDate nextOccurrenceDate = command.startDate() != null || command.dayOfMonth() != null
                ? firstOccurrence(max(startDate, current.nextOccurrenceDate()), dayOfMonth) : current.nextOccurrenceDate();
        return recurringTransactionRepository.save(new RecurringTransaction(current.id(), current.userId(), categoryId,
                command.amount() == null ? current.amount() : command.amount(),
                command.currency() == null ? current.currency() : command.currency(),
                command.exchangeRateToBase() == null ? current.exchangeRateToBase() : command.exchangeRateToBase(),
                command.description() == null ? current.description() : strip(command.description()), transactionType,
                dayOfMonth, startDate, nextOccurrenceDate, active, current.createdAt(), clock.instant(), current.version()));
    }

    @Transactional
    public void delete(UUID userId, UUID id, long version) {
        var current = findOwned(userId, id);
        if (current.version() != version) throw new OptimisticLockingFailureException("Recurring transaction has been modified.");
        recurringTransactionRepository.delete(current);
    }

    @Transactional
    public void createDueOccurrences(LocalDate businessDate) {
        recurringTransactionRepository.findActiveDueOnOrBefore(businessDate).forEach(rule -> createDueOccurrences(rule, businessDate));
    }
    ...
    private void createDueOccurrences(RecurringTransaction rule, LocalDate businessDate) {
        ...
        while (createdOccurrences < MAX_OCCURRENCES_PER_RULE_PER_RUN
            ...
            Transaction transaction = transactionRepository.save(new Transaction(transactionId, updated.userId(), updated.categoryId(), updated.amount(),
                    updated.currency(), updated.exchangeRateToBase(), updated.nextOccurrenceDate(), updated.description(),
                    updated.transactionType(), updated.id(), now, now, 0));
            ...
            updated = new RecurringTransaction(updated.id(), updated.userId(), updated.categoryId(), updated.amount(),
                    updated.currency(), updated.exchangeRateToBase(), updated.description(), updated.transactionType(),
                    updated.dayOfMonth(), updated.startDate(), nextOccurrence(updated.nextOccurrenceDate(), updated.dayOfMonth()),
                    updated.active(), updated.createdAt(), now, updated.version());
    ...
    private void requireMatchingCategory(UUID userId, UUID categoryId, TransactionType transactionType) {
        var category = categoryRepository.findByIdAndUserId(categoryId, userId).orElseThrow(ResourceNotFoundException::new);
        if (category.transactionType() != transactionType) throw new RecurringTransactionCategoryTypeMismatchException();
    }
    private RecurringTransaction findOwned(UUID userId, UUID id) {
        return recurringTransactionRepository.findByIdAndUserId(id, userId).orElseThrow(ResourceNotFoundException::new);
    }
    private LocalDate firstOccurrence(LocalDate startDate, int dayOfMonth) {
        ...
        return candidate.isBefore(startDate) ? occurrenceInMonth(YearMonth.from(startDate).plusMonths(1), dayOfMonth) : candidate;
    }
    ...
    private LocalDate occurrenceInMonth(YearMonth month, int dayOfMonth) { return month.atDay(Math.min(dayOfMonth, month.lengthOfMonth())); }
    private LocalDate max(LocalDate first, LocalDate second) { return first.isAfter(second) ? first : second; }
    private String strip(String description) { return description == null ? null : description.strip(); }
}
