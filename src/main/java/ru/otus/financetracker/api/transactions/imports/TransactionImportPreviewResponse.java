...
public record TransactionImportPreviewResponse(List<Row> rows, List<LineError> lineErrors) {
    ...
    public static TransactionImportPreviewResponse from(
        ...
        return new TransactionImportPreviewResponse(
                preview.rows().stream().map(row -> new Row(row.lineNumber(), row.categoryId(), row.amount(), row.currency(),
                        row.exchangeRateToBase(), row.transactionDate(), row.description(), row.transactionType())).toList(),
                preview.lineErrors().stream().map(error -> new LineError(error.lineNumber(), error.field(), error.code(),
