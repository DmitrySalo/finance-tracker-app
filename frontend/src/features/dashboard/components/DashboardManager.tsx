...
function PieChart({ categories }: { categories: DashboardCategoryExpense[] }) {
  ...
  const alternative = categories.map((category) => `${category.categoryName}: ${formatDecimal(category.amount)}`).join("; ");
  ...
  return <figure className={styles.chartCard}>
    ...
    <ul className={styles.legend}>{categories.map((category, index) => <li key={category.categoryId}><span aria-hidden="true" className={styles.legendMarker} style={{ backgroundColor: categoryColor(category, index) }} />{category.categoryIcon} {category.categoryName}: {formatDecimal(category.amount)}</li>)}</ul>
  </figure>;
}
...
function LineChart({ trend }: { trend: SpendingTrend }) {
  ...
  const alternative = trend.months.map((entry) => `${entry.month}: ${formatDecimal(entry.amount)}`).join("; ");
  ...
  return <figure className={styles.chartCard}>
    ...
    <ul className={styles.trendValues}>{trend.months.map((entry) => <li key={entry.month}><span>{entry.month}</span><strong>{formatDecimal(entry.amount)}</strong></li>)}</ul>
  </figure>;
}
