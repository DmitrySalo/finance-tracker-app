...
test("shows before and after states and applies the audit filters", async () => {
  const auditPage = { items: [{ id: "123e4567-e89b-42d3-a456-426614174003", entityType: "BUDGET", entityId: "123e4567-e89b-42d3-a456-426614174004", action: "UPDATE", occurredAt: "2026-09-17T10:00:00Z", beforeState: { limitAmount: "100.0000", exchangeRateToBase: "1.08500000", currency: "USD" }, afterState: { limitAmount: "125.0000", exchangeRateToBase: "1.23456789", currency: "USD" } }], page: { number: 0, size: 20, totalElements: 1, totalPages: 1 } };
  ...
  await waitFor(() => expect(fetchMock.mock.calls.some(([input]) => String(input).includes("entityType=BUDGET") && String(input).includes("entityId=123e4567-e89b-42d3-a456-426614174004"))).toBe(true)); expect(await screen.findByText("100.00")).toBeTruthy(); expect(screen.getByText("125.00")).toBeTruthy(); expect(screen.getByText("1.08500000")).toBeTruthy(); expect(screen.getByText("1.23456789")).toBeTruthy();
});
